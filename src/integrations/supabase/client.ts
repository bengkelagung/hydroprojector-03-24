...
export const fetchPinConfigsWithRelations = async (userId: string) => {
  try {
    ...
    return pinConfigs.map(pc => ({
      ...
      pin_id: pc.pin_id,
      pin_number: pc.pins[0].pin_number,
      pin_name: pc.pins[0].pin_name,
      data_type_id: pc.data_type_id,
      data_type_name: pc.data_types[0].name,
      signal_type_id: pc.signal_type_id,
      signal_type_name: pc.signal_types[0].name,
      mode_id: pc.mode_id,
      mode_type: pc.modes[0].type,
      label_id: pc.label_id,
      label_name: pc.label?.[0]?.name || null,
      name: pc.name,
      unit: pc.unit || '',
      value: pc.value || '',
...
/**
 * Get pin history data for charts - returns all data points
 * @param pinId - ID of the pin
 * @param hours - Number of hours of history to fetch
 * @returns Array of all history data points
 */
